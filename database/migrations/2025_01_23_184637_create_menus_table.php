...
class CreateMenusTable extends Migration
{
    public function up()
    {
        Schema::create('menus', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('category_menus_id'); // Foreign key for Menu Category
            $table->string('image')->nullable(); // Image URL for the menu item
            $table->boolean('is_available')->default(true); // Availability status
            $table->timestamps();

            // Define the foreign key constraint
            $table->foreign('category_menus_id')->references('id')->on('category_menus')->onDelete('cascade');
        });
    }
